...
with col3:
    ...
    with col_limit1:
        limit_piutang = st.text_input("Limit Piutang (Rp)", value=get_val('limit_piutang'))
    with col_limit2:
        limit_nota = st.text_input("Limit Lembar Nota (Rp)", value=get_val('limit_nota'))
...
# --- UI BAGIAN 4: LAMPIRAN FOTO & DOKUMEN ---
st.subheader("4. Lampiran Foto & Dokumen")
st.info("💡 Unggah gambar pendukung di sini sebelum klik submit.")

col_img1, col_img2 = st.columns(2)
with col_img1:
    ktp_img = st.file_uploader("Upload Foto KTP", type=['jpg', 'jpeg', 'png'])
    depan_img = st.file_uploader("Foto Toko: Tampak Depan (Terlihat Jalan)", type=['jpg', 'jpeg', 'png'])
    kiri_img = st.file_uploader("Foto Toko: Tampak Samping Kiri", type=['jpg', 'jpeg', 'png'])

with col_img2:
    npwp_img = st.file_uploader("Upload Foto NPWP (Opsional)", type=['jpg', 'jpeg', 'png'])
    dalam_img = st.file_uploader("Foto Toko: Tampak Dalam", type=['jpg', 'jpeg', 'png'])
    kanan_img = st.file_uploader("Foto Toko: Tampak Samping Kanan", type=['jpg', 'jpeg', 'png'])


# --- UI BAGIAN 5: DIGITAL SIGNATURE ---
st.markdown("---")
...
with col_sig1:
    st.write(f"**Tanda Tangan Pelanggan ({nama_pemilik if nama_pemilik else '...'})**")
...
with col_sig2:
    st.write(f"**Tanda Tangan Sales ({nama_sales if nama_sales else '...'})**")
...
with col_btn1:
    if st.button("🗑️ Hapus Draft & Reset"):
        cookie_manager.delete("noo_draft")
        st.rerun()
